...
export async function sendMessage(
  ...
  await channelRepo.updateChannelLastMessage(
    channelId,
    input.content?.slice(0, 100) ?? "[attachment]"
  );
...
): Promise<{ messages: Message[]; meta: any }> {
  const channel = await channelRepo.getChannelById(channelId);
  if (!channel || channel.workspaceId !== workspaceId) {
    throw new AppError("Channel not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
  }

  return messageRepo.getMessagesByChannel(
    channelId,
    workspaceId,
    query.limit ?? 50,
    query.cursor
...
export async function addReaction(
  ...
  if (totalReactions >= MESSAGE_LIMITS.MAX_REACTIONS_PER_MESSAGE) {
    throw new AppError(
      `Maximum of ${MESSAGE_LIMITS.MAX_REACTIONS_PER_MESSAGE} reactions per message`,
...
export async function searchMessages(
  workspaceId: string,
  query: SearchMessagesQueryInput
