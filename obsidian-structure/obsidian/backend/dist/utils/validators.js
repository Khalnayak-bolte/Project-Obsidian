...
const requiredString = (label, max = 1000) => z
    .string({ required_error: `${label} is required` })
    .trim()
    .min(1, `${label} cannot be empty`)
    .max(max, `${label} must be at most ${max} characters`);
/** Optional trimmed string — strips empty strings to undefined. */
const optionalString = (max = 1000) => z.string().trim().max(max).optional().transform((v) => v || undefined);
...
const obsidianId = (prefix) => z
    .string()
    .regex(new RegExp(`^${prefix}_[A-Za-z0-9]{6,32}$`), `Invalid ${prefix} ID format`);
/** Email address. */
...
const httpsUrl = z
    ...
    .refine((url) => process.env.NODE_ENV !== "production" || url.startsWith("https://"), "URL must use HTTPS");
...
export const CreateChannelSchema = z.object({
    name: z
        ...
        .trim()
        .toLowerCase()
        .regex(/^[a-z0-9-_]{1,80}$/, "Channel name must be lowercase alphanumeric with hyphens/underscores"),
...
}).refine((d) => (d.content && d.content.length > 0) || (d.attachments && d.attachments.length > 0), { message: "Message must have content or at least one attachment" });
...
// ─── FILE schemas ─────────────────────────────────────────────────────────────
const AllowedMimeTypes = z.string().refine((mime) => {
...
export const RequestUploadUrlSchema = z.object({
    ...
    sizeBytes: positiveInt.max(500 * 1024 * 1024, // 500 MB hard cap; tier limits enforced server-side
...
export function validate(schema, options = {}) {
    ...
    return (req, res, next) => {
        ...
        const data = stripUnknown
            ? schema.parse(raw) // parse() always strips with z.object()
            : parseResult.data;
        req.validated = data;
        next();
    };
