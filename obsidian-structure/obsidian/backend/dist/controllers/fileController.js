...
import { RequestUploadUrlSchema, ConfirmUploadSchema, BulkDeleteFilesSchema, ListFilesQuerySchema, SearchFilesQuerySchema, } from "../schemas/file.schema";
import { requestUploadUrl as requestUploadUrlService, confirmUpload as confirmUploadService, getDownloadUrl as getDownloadUrlService, deleteFile as deleteFileService, bulkDeleteFiles as bulkDeleteFilesService, listFiles as listFilesService, getStorageUsage as getStorageUsageService, } from "../services/fileService";
...
export async function requestUploadUrl(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(result, "Upload URL generated. Use it to PUT your file directly to S3."));
    }
    catch (err) {
        next(err);
    }
...
export async function confirmUpload(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.CREATED).json(successResponse(file, "File upload confirmed and stored."));
    }
    catch (err) {
        next(err);
    }
...
export async function listFiles(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(result.files, undefined, {
            limit: query.limit,
...
export async function getDownloadUrl(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(result, "Download URL generated."));
    }
    catch (err) {
        next(err);
    }
...
export async function deleteFile(req, res, next) {
    try {
        ...
        const workspaceId = authedReq.user.workspaceId;
        const { fileId } = req.params;
        await deleteFileService(workspaceId, uid, fileId, authedReq.user.permissions);
        logger.info("File deleted", { fileId, uid, workspaceId });
        res.status(HTTP_STATUS.OK).json(successResponse(null, "File deleted successfully."));
    }
    catch (err) {
        next(err);
    }
...
export async function bulkDeleteFiles(req, res, next) {
    try {
        ...
        const result = await bulkDeleteFilesService(workspaceId, uid, body, authedReq.user.permissions);
        logger.info("Bulk file delete complete", {
            workspaceId,
            deleted: result.deleted,
            failed: result.failed,
        });
        res.status(HTTP_STATUS.OK).json(successResponse(result, `${result.deleted} file(s) deleted${result.failed ? `, ${result.failed} failed` : ""}.`));
    }
    catch (err) {
        next(err);
    }
