...
import { listMembers as listMembersService, removeMember as removeMemberService, getWorkspaceStats as getWorkspaceStatsService, } from "../services/workspaceService";
...
async function writeAuditLog(workspaceId, uid, action, meta = {}) {
    try {
...
export async function getAuditLog(req, res, next) {
    try {
        ...
        if (query.action) {
            ref = ref.where("action", "==", query.action);
        }
        if (query.uid) {
            ref = ref.where("actorId", "==", query.uid);
        }
        if (query.cursor) {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse({ entries, hasMore, nextCursor }, "Audit log retrieved."));
    }
    catch (err) {
...
export async function banMember(req, res, next) {
    try {
        ...
        if (targetUid === actorId) {
            throw new AppError("You cannot ban yourself.", HTTP_STATUS.BAD_REQUEST, ERROR_CODES.BAD_REQUEST);
        }
        const targetUser = await getUserById(targetUid);
        if (!targetUser || targetUser.workspaceId !== workspaceId) {
            throw new AppError("Member not found in this workspace.", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
        }
        if (targetUser.roleId === SYSTEM_ROLES.OWNER) {
            throw new AppError("Workspace owner cannot be banned.", HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
        }
...
export async function removeMember(req, res, next) {
    try {
        ...
        const targetUid = req.params.uid;
        if (targetUid === actorId) {
            throw new AppError("You cannot remove yourself.", HTTP_STATUS.BAD_REQUEST, ERROR_CODES.BAD_REQUEST);
        }
...
export async function createRole(req, res, next) {
    try {
        ...
        if (existingSnap.size >= maxRoles) {
            throw new AppError(`Your plan allows a maximum of ${maxRoles} custom roles.`, HTTP_STATUS.FORBIDDEN, ERROR_CODES.TIER_LIMIT_EXCEEDED);
        }
...
export async function updateRole(req, res, next) {
    try {
        ...
        if (roleSnap.data()?.isSystem) {
            throw new AppError("System roles cannot be modified.", HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
        }
        ...
        if (body.hoistable !== undefined)
            updates.hoistable = body.hoistable;
...
export async function deleteRole(req, res, next) {
    try {
        ...
        if (roleSnap.data()?.isSystem) {
            throw new AppError("System roles cannot be deleted.", HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
        }
...
export async function assignRole(req, res, next) {
    try {
        ...
        if (targetUser.roleId === SYSTEM_ROLES.OWNER) {
            throw new AppError("Workspace owner role cannot be reassigned via this endpoint. Use transfer ownership.", HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
        }
        // Verify role exists in this workspace (or is a system role)
        const isSystemRole = Object.values(SYSTEM_ROLES).includes(body.roleId);
        if (!isSystemRole) {
        ...
        await updateUserProfile(targetUid, { roleId: body.roleId });
...
export async function bulkDeleteMessages(req, res, next) {
    try {
        ...
        if (!channelSnap.exists || channelSnap.data()?.workspaceId !== workspaceId) {
            throw new AppError("Channel not found in this workspace.", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
        }
        ...
        res.status(HTTP_STATUS.OK).json(successResponse({ deleted: body.messageIds.length, channelId: body.channelId }, `${body.messageIds.length} message(s) deleted.`));
    }
    catch (err) {
