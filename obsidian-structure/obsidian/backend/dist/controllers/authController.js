...
import { RegisterWithEmailSchema, OAuthLoginSchema, LogoutSchema, RequestPasswordResetSchema, UpdateProfileSchema, VerifyTokenSchema, } from "../schemas/auth.schema";
import { registerWithEmail, handleOAuthLogin, verifyFirebaseToken, getUserSession, revokeDeviceSession, revokeAllSessions, sendPasswordReset, isEmailRegistered, } from "../services/authService";
...
export async function register(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.CREATED).json(successResponse(result, "Account created successfully."));
    }
    catch (err) {
        next(err);
    }
...
export async function oauthLogin(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(result, "Logged in successfully."));
    }
    catch (err) {
        next(err);
    }
...
export async function logout(req, res, next) {
    try {
        ...
        if (workspaceId) {
            await setPresence({ uid, workspaceId, status: "offline" }).catch((err) => logger.warn("Failed to set presence on logout", { uid, err }));
        }
...
export async function requestPasswordReset(req, res, next) {
    try {
        ...
        await sendPasswordReset(email).catch((err) => logger.warn("Password reset generation failed silently", { email, err }));
        res.status(HTTP_STATUS.OK).json(successResponse(null, "If an account exists for this email, a reset link has been sent."));
    }
    catch (err) {
        next(err);
    }
...
export async function updateProfile(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(updated, "Profile updated successfully."));
    }
    catch (err) {
        next(err);
    }
...
export async function revokeSession(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(null, "Session revoked successfully."));
    }
    catch (err) {
        next(err);
    }
...
export async function checkEmail(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse({ available: !exists }));
    }
    catch (err) {
        next(err);
    }
