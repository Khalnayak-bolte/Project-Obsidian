...
import { CreateOrderSchema, VerifyPaymentSchema, CancelSubscriptionSchema, ReactivateSubscriptionSchema, ChangePlanSchema, ApplyCouponSchema, RequestRefundSchema, RazorpayWebhookSchema, ListInvoicesQuerySchema, BillingDashboardQuerySchema, } from "../schemas/payment.schema";
import { createOrder as createOrderService, verifyPayment as verifyPaymentService, cancelSubscription as cancelSubscriptionService, changePlan as changePlanService, getSubscription, listInvoices as listInvoicesService, handleWebhook as handleWebhookService, } from "../services/billingService";
...
export async function createOrder(req, res, next) {
    try {
        ...
        if (body.workspaceId !== workspaceId) {
            throw new AppError("Workspace ID mismatch.", HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
        }
...
export async function getBillingDashboard(req, res, next) {
    try {
        const authedReq = req;
        const workspaceId = authedReq.user.workspaceId;
        ...
        if (query.workspaceId !== workspaceId) {
            throw new AppError("You can only view billing for your own workspace.", HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
        }
...
export async function applyCoupon(req, res, next) {
    try {
        ...
        throw new AppError("Coupon code not found or expired.", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
    catch (err) {
...
export async function requestRefund(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse({ paymentId: body.paymentId, status: "pending_review" }, "Refund request submitted. Our team will review it within 3-5 business days."));
    }
    catch (err) {
...
export async function handleWebhook(req, res, next) {
    try {
        ...
        if (!signature) {
            throw new AppError("Missing Razorpay webhook signature.", HTTP_STATUS.BAD_REQUEST, ERROR_CODES.INVALID_WEBHOOK);
        }
        ...
        try {
            ...
            throw new AppError("Invalid JSON in webhook payload.", HTTP_STATUS.BAD_REQUEST, ERROR_CODES.INVALID_WEBHOOK);
        }
