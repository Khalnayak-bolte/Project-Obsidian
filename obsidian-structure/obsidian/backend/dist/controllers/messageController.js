...
import { SendMessageSchema, EditMessageSchema, AddReactionSchema, BulkDeleteMessagesSchema, GetMessagesQuerySchema, SearchMessagesQuerySchema, } from "../schemas/message.schema";
import { sendMessage as sendMessageService, getMessages as getMessagesService, getMessagesAround as getMessagesAroundService, editMessage as editMessageService, deleteMessage as deleteMessageService, bulkDeleteMessages as bulkDeleteMessagesService, addReaction as addReactionService, removeReaction as removeReactionService, pinMessage as pinMessageService, unpinMessage as unpinMessageService, getPinnedMessages as getPinnedMessagesService, searchMessages as searchMessagesService, } from "../services/messageService";
...
export async function sendMessage(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.CREATED).json(successResponse(message, "Message sent."));
    }
    catch (err) {
        next(err);
    }
...
export async function getMessages(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(result.messages, undefined, result.meta));
    }
    catch (err) {
        next(err);
    }
...
export async function getMessagesAround(req, res, next) {
    try {
        ...
        const messages = await getMessagesAroundService(workspaceId, channelId, messageId);
        res.status(HTTP_STATUS.OK).json(successResponse(messages));
    }
    catch (err) {
        next(err);
...
export async function searchMessages(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(result.messages, undefined, {
            limit: query.limit,
            hasMore: false,
            total: result.total,
        }));
...
export async function editMessage(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(updated, "Message updated."));
    }
    catch (err) {
        next(err);
    }
...
export async function deleteMessage(req, res, next) {
    try {
        ...
        const workspaceId = authedReq.user.workspaceId;
        const { messageId } = req.params;
        await deleteMessageService(workspaceId, messageId, uid, authedReq.user.permissions);
        logger.info("Message deleted", { messageId, uid });
        res.status(HTTP_STATUS.OK).json(successResponse(null, "Message deleted."));
    }
    catch (err) {
        next(err);
    }
...
export async function bulkDeleteMessages(req, res, next) {
    try {
        ...
        const result = await bulkDeleteMessagesService(workspaceId, channelId, uid, body);
        logger.info("Bulk delete complete", { channelId, deleted: result.deleted });
        res.status(HTTP_STATUS.OK).json(successResponse(result, `${result.deleted} message(s) deleted.`));
    }
    catch (err) {
        next(err);
    }
...
export async function addReaction(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse({ emoji: body.emoji }, "Reaction added."));
    }
    catch (err) {
        next(err);
    }
...
export async function removeReaction(req, res, next) {
    try {
        ...
        if (!emoji) {
            res.status(HTTP_STATUS.BAD_REQUEST).json(successResponse(null, "emoji query param is required."));
            return;
        }
        await removeReactionService(workspaceId, messageId, uid, emoji);
        res.status(HTTP_STATUS.OK).json(successResponse({ emoji }, "Reaction removed."));
    }
    catch (err) {
        next(err);
    }
...
export async function pinMessage(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(null, "Message pinned."));
    }
    catch (err) {
        next(err);
    }
...
export async function unpinMessage(req, res, next) {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(null, "Message unpinned."));
    }
    catch (err) {
        next(err);
    }
