...
export async function requestUploadUrl(workspaceId, uid, input) {
    ...
    if (input.sizeBytes > limits.maxFileSizeBytes) {
        ...
        throw new AppError(`File size exceeds the ${maxMb}MB limit for your plan`, HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
    }
    ...
    if (storageUsed + input.sizeBytes > limits.maxStorageBytes) {
        throw new AppError("Workspace storage limit reached. Please upgrade your plan or delete unused files.", HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
    }
...
export async function confirmUpload(workspaceId, uid, input) {
    const workspace = await workspaceRepo.getWorkspaceById(workspaceId);
    if (!workspace) {
        throw new AppError("Workspace not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
...
export async function getDownloadUrl(workspaceId, fileId) {
...
// ─── Delete file ──────────────────────────────────────────────────────────────
export async function deleteFile(workspaceId, actorId, fileId, actorPermissions) {
    const file = await fileRepo.getFileById(fileId);
    if (!file || file.workspaceId !== workspaceId) {
        throw new AppError("File not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
    ...
    // Delete from S3
    try {
        await s3Client.send(new DeleteObjectCommand({ Bucket: S3_CONFIG.bucket, Key: file.s3Key }));
    }
    catch (err) {
...
// ─── Bulk delete files ────────────────────────────────────────────────────────
export async function bulkDeleteFiles(workspaceId, actorId, input, actorPermissions) {
    let deleted = 0;
    let failed = 0;
...
export async function cleanupOrphanedFiles(workspaceId) {
