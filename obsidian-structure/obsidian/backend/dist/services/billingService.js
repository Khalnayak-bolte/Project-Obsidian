...
import { razorpay, RAZORPAY_PRICING, RAZORPAY_WEBHOOK, RAZORPAY_CURRENCY, getPlanId, getPrice, getTierFromPlanId, isValidTier, } from "../config/razorpay";
...
export async function createOrder(workspaceId, uid, input) {
    ...
    const amount = getPrice(input.tier, input.interval);
    const planId = getPlanId(input.tier, input.interval);
    if (!planId) {
        throw new AppError(`Razorpay plan not configured for ${input.tier}/${input.interval}`, HTTP_STATUS.INTERNAL_SERVER_ERROR, ERROR_CODES.INTERNAL_ERROR);
    }
...
export async function verifyPayment(workspaceId, uid, input) {
    // HMAC signature verification
    const secret = process.env.RAZORPAY_KEY_SECRET ?? "";
    const isValid = verifyRazorpayPaymentSignature(input.razorpayOrderId, input.razorpayPaymentId, input.razorpaySignature, secret);
    if (!isValid) {
...
// ─── Cancel subscription ──────────────────────────────────────────────────────
export async function cancelSubscription(workspaceId, uid, input) {
...
// ─── Change plan ──────────────────────────────────────────────────────────────
export async function changePlan(workspaceId, uid, input) {
    const workspace = await workspaceRepo.getWorkspaceById(workspaceId);
    if (!workspace) {
        throw new AppError("Workspace not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
...
// ─── List invoices ────────────────────────────────────────────────────────────
export async function listInvoices(workspaceId, query) {
...
export async function handleWebhook(rawBody, signature, payload) {
    ...
    // 5. Route event
    try {
        await routeWebhookEvent(payload.event, payload.payload);
