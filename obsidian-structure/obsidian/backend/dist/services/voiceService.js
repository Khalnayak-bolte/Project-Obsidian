...
import { chimeClient, CHIME_CONFIG, buildExternalMeetingId, buildExternalUserId, CreateMeetingCommand, CreateAttendeeCommand, DeleteMeetingCommand, GetMeetingCommand, } from "../config/chime";
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
async function getOrCreateChimeMeeting(workspaceId, channelId, voiceQuality) {
    ...
    if (existing?.chimeMeetingId) {
        try {
            const res = await chimeClient.send(new GetMeetingCommand({ MeetingId: existing.chimeMeetingId }));
            if (res.Meeting) {
                return {
    ...
    const preset = CHIME_CONFIG.qualityPresets[voiceQuality]
        ?? CHIME_CONFIG.qualityPresets.standard;
    const res = await chimeClient.send(new CreateMeetingCommand({
        ClientRequestToken: externalMeetingId,
        ExternalMeetingId: externalMeetingId,
        MediaRegion: CHIME_CONFIG.mediaRegion,
        MeetingFeatures: preset.echoReduction
...
// ─── Join voice channel ───────────────────────────────────────────────────────
export async function joinVoice(workspaceId, uid, displayName, input) {
    ...
    const { meetingId, mediaPlacement } = await getOrCreateChimeMeeting(workspaceId, input.channelId, voiceQuality);
...
// ─── Leave voice channel ──────────────────────────────────────────────────────
export async function leaveVoice(workspaceId, uid, input) {
...
export async function endVoiceSession(sessionId, workspaceId) {
    ...
    try {
        await chimeClient.send(new DeleteMeetingCommand({ MeetingId: session.chimeMeetingId }));
    }
    catch (err) {
...
export async function updateParticipantState(workspaceId, uid, sessionId, input) {
    const session = await voiceRepo.getVoiceSessionById(sessionId);
    if (!session || session.workspaceId !== workspaceId) {
        throw new AppError("Voice session not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
...
export async function muteParticipant(workspaceId, actorId, input) {
...
// ─── Kick participant ─────────────────────────────────────────────────────────
export async function kickParticipant(workspaceId, actorId, input) {
    const session = await voiceRepo.getActiveSessionByChannel(input.channelId);
    if (!session || session.workspaceId !== workspaceId) {
        throw new AppError("No active voice session in this channel", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
...
// ─── Voice heartbeat ──────────────────────────────────────────────────────────
export async function voiceHeartbeat(workspaceId, uid, input) {
...
export async function getActiveSession(workspaceId, channelId) {
...
export async function getVoicePresence(workspaceId, channelId) {
    const channel = await channelRepo.getChannelById(channelId);
    if (!channel || channel.workspaceId !== workspaceId) {
        throw new AppError("Channel not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
