...
// ─── Create channel ───────────────────────────────────────────────────────────
export async function createChannel(workspaceId, actorId, input) {
    ...
    if (existing.length >= limits.maxChannels) {
        throw new AppError(`Channel limit of ${limits.maxChannels} reached for your plan. Please upgrade.`, HTTP_STATUS.FORBIDDEN, ERROR_CODES.FORBIDDEN);
    }
    // Name uniqueness within workspace + type
    const taken = await channelRepo.channelExists(workspaceId, input.name, input.type);
    if (taken) {
        throw new AppError(`A ${input.type} channel named "${input.name}" already exists`, HTTP_STATUS.CONFLICT, ERROR_CODES.CONFLICT);
    }
...
// ─── Get channel ──────────────────────────────────────────────────────────────
export async function getChannel(channelId, workspaceId) {
...
// ─── List channels ────────────────────────────────────────────────────────────
export async function listChannels(workspaceId, roleId, opts = {}) {
    const workspace = await workspaceRepo.getWorkspaceById(workspaceId);
    if (!workspace) {
        throw new AppError("Workspace not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
    if (opts.type) {
        return channelRepo.getChannelsByType(workspaceId, opts.type);
    }
    return channelRepo.getAccessibleChannels(workspaceId, roleId);
}
// ─── Update channel ───────────────────────────────────────────────────────────
export async function updateChannel(channelId, workspaceId, actorId, input) {
    const channel = await channelRepo.getChannelById(channelId);
    if (!channel || channel.workspaceId !== workspaceId) {
        throw new AppError("Channel not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
    ...
    if (input.name && input.name !== channel.name) {
        ...
        if (taken) {
            throw new AppError(`A channel named "${input.name}" already exists`, HTTP_STATUS.CONFLICT, ERROR_CODES.CONFLICT);
        }
    }
...
// ─── Delete channel ───────────────────────────────────────────────────────────
export async function deleteChannel(channelId, workspaceId, actorId) {
    const channel = await channelRepo.getChannelById(channelId);
    if (!channel || channel.workspaceId !== workspaceId) {
        throw new AppError("Channel not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
...
// ─── Archive channel ──────────────────────────────────────────────────────────
export async function archiveChannel(channelId, workspaceId, actorId, isArchived) {
    const channel = await channelRepo.getChannelById(channelId);
    if (!channel || channel.workspaceId !== workspaceId) {
        throw new AppError("Channel not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
...
// ─── Reorder channels ─────────────────────────────────────────────────────────
export async function reorderChannels(workspaceId, actorId, input) {
    ...
    for (const { channelId } of input.order) {
        if (!workspaceChannelIds.has(channelId)) {
            throw new AppError(`Channel ${channelId} does not belong to this workspace`, HTTP_STATUS.BAD_REQUEST, ERROR_CODES.BAD_REQUEST);
        }
    }
...
export async function markChannelRead(channelId, workspaceId, uid, _input) {
    const channel = await channelRepo.getChannelById(channelId);
    if (!channel || channel.workspaceId !== workspaceId) {
        throw new AppError("Channel not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
...
export async function updateChannelPermissions(channelId, workspaceId, actorId, input) {
    const channel = await channelRepo.getChannelById(channelId);
    if (!channel || channel.workspaceId !== workspaceId) {
        throw new AppError("Channel not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
...
export async function addChannelMembers(channelId, workspaceId, actorId, input) {
    const channel = await channelRepo.getChannelById(channelId);
    if (!channel || channel.workspaceId !== workspaceId) {
        throw new AppError("Channel not found", HTTP_STATUS.NOT_FOUND, ERROR_CODES.NOT_FOUND);
    }
    if (channel.visibility !== "private") {
        throw new AppError("Members can only be explicitly added to private channels", HTTP_STATUS.BAD_REQUEST, ERROR_CODES.BAD_REQUEST);
    }
...
// ─── Get voice channels ───────────────────────────────────────────────────────
export async function getVoiceChannels(workspaceId) {
