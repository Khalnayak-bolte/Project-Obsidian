...
import { requestUploadUrl, confirmUpload, listFiles, searchFiles, getStorageUsage, getFile, getDownloadUrl, deleteFile, bulkDeleteFiles, } from "../controllers/fileController";
export const fileRouter = Router();
// All file routes require authentication
fileRouter.use(authenticate);
// ─── Upload pipeline ──────────────────────────────────────────────────────────
// POST /api/v1/files/upload-url  — request a signed S3 URL for direct client upload
fileRouter.post("/upload-url", requirePermission("upload_files"), RATE_LIMITS.UPLOADS, requestUploadUrl);
// POST /api/v1/files/confirm  — notify backend that S3 upload completed
fileRouter.post("/confirm", requirePermission("upload_files"), confirmUpload);
// ─── Listing & search ─────────────────────────────────────────────────────────
...
fileRouter.get("/search", requirePermission("download_files"), RATE_LIMITS.SEARCH, searchFiles);
...
fileRouter.get("/:fileId/download", requirePermission("download_files"), getDownloadUrl);
// DELETE /api/v1/files/:fileId
fileRouter.delete("/:fileId", deleteFile);
// ─── Bulk operations ──────────────────────────────────────────────────────────
// POST /api/v1/files/bulk-delete
fileRouter.post("/bulk-delete", requirePermission("delete_files"), bulkDeleteFiles);
