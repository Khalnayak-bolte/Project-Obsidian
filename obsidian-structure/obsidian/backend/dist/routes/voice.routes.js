...
import { joinVoice, leaveVoice, getActiveSession, getVoicePresence, updateParticipantState, muteParticipant, kickParticipant, endVoiceSession, heartbeat, reportQualityStats, } from "../controllers/voiceController";
export const voiceRouter = Router();
// All voice routes require authentication
voiceRouter.use(authenticate);
// ─── Join / Leave ─────────────────────────────────────────────────────────────
// POST /api/v1/voice/join
voiceRouter.post("/join", requirePermission("join_voice"), RATE_LIMITS.VOICE_JOIN, joinVoice);
...
voiceRouter.post("/mute/:uid", requirePermission("mute_members"), muteParticipant);
// POST /api/v1/voice/kick/:uid  (moderator: kick participant from voice)
voiceRouter.post("/kick/:uid", requirePermission("kick_members"), kickParticipant);
// ─── Session management ───────────────────────────────────────────────────────
// POST /api/v1/voice/end  (admin: forcefully end a voice session)
voiceRouter.post("/end", requirePermission("manage_channels"), endVoiceSession);
// ─── Heartbeat & Quality ──────────────────────────────────────────────────────
