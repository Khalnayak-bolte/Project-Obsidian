...
export const getPrice = (
  tier: keyof typeof RAZORPAY_PRICING,
  interval: "monthly" | "yearly"
): number => {
...
export const getTierFromPlanId = (planId: string): string | null => {
  const entry = Object.entries(RAZORPAY_PLANS).find(
    ([, id]) => id === planId
  );
...
export const isValidTier = (tier: string): tier is keyof typeof appConfig.subscription.tiers => {
