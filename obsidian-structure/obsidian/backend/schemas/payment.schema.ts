...
const razorpayIdField = (label: string) =>
  z
...
export const ListInvoicesQuerySchema = z.object({
  ...
  status: z
    .enum(["draft", "paid", "void", "uncollectible"], {
      errorMap: () => ({
        message:
          "Invoice status must be one of: draft, paid, void, uncollectible.",
      }),
    })
    .optional(),
});
...
export type BillingDashboardQueryInput = z.infer<
...
export const ApplyCouponSchema = z.object({
  ...
  couponCode: z
    ...
    .regex(
      /^[A-Z0-9_-]+$/,
      "Coupon code may only contain uppercase letters, numbers, underscores, and hyphens."
    ),
});
