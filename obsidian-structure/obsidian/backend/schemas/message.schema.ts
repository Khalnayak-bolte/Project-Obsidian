...
export const MessageTypeEnum = z.enum(
  ["text", "system", "file", "voice_started", "voice_ended"],
  {
    errorMap: () => ({
      message:
        "Message type must be one of: text, system, file, voice_started, voice_ended.",
    }),
  }
);
...
const emojiRegex =
  /^(\p{Emoji_Presentation}|\p{Extended_Pictographic})(\u200D(\p{Emoji_Presentation}|\p{Extended_Pictographic})|\uFE0F|\u20E3)*$/u;
...
export const GetMessagesQuerySchema = z
  ...
  .refine(
    ...
    { message: "Cannot use both 'before' and 'after' simultaneously." }
  );
