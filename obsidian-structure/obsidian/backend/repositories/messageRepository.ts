...
export async function getMessageById(
  ...
  try {
    ...
    return { messageId: snap.id, ...snap.data() } as Message;
  } catch (err) {
...
export async function addReaction(
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (existing) {
        if (existing.reactedBy.includes(uid)) return; // already reacted
...
export async function removeReaction(
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      const data = snap.data() as Message;
      let reactions: MessageReaction[] = data.reactions ?? [];
...
export async function pinMessage(
  ...
  try {
    ...
    const message = { messageId, ...messageSnap.data() } as Message;
    const now = Timestamp.now();
...
export async function getPinnedMessages(
  ...
  try {
    ...
    return snap.docs.map((doc) => doc.data()) as PinnedMessage[];
  } catch (err) {
...
export async function searchMessages(
  ...
  try {
    ...
    const results = snap.docs
      .map((doc) => ({ messageId: doc.id, ...doc.data() }) as Message)
