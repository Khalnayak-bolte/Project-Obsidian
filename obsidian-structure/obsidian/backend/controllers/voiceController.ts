...
import {
  ...
  UpdateParticipantStateSchema,
  VoiceHeartbeatSchema,
  VoiceQualityStatsSchema,
  EndVoiceSessionSchema,
} from "../schemas/voice.schema";
import {
  joinVoice as joinVoiceService,
  leaveVoice as leaveVoiceService,
  getActiveSession as getActiveSessionService,
  getVoicePresence as getVoicePresenceService,
  updateParticipantState as updateParticipantStateService,
  muteParticipant as muteParticipantService,
  kickParticipant as kickParticipantService,
  endVoiceSession as endVoiceSessionService,
  voiceHeartbeat as voiceHeartbeatService,
...
export async function joinVoice(
  ...
  try {
    ...
    res.status(HTTP_STATUS.OK).json(
      successResponse(result, "Joined voice channel.")
    );
  } catch (err) {
    next(err);
  }
...
export async function leaveVoice(
  ...
  try {
    ...
    res.status(HTTP_STATUS.OK).json(
      successResponse(null, "Left voice channel.")
    );
  } catch (err) {
    next(err);
  }
  ...
  try {
    ...
    if (!sessionId) {
      res.status(HTTP_STATUS.BAD_REQUEST).json(
        successResponse(null, "sessionId is required in the request body.")
      );
      return;
    }
    ...
    res.status(HTTP_STATUS.OK).json(
      successResponse(body, "Participant state updated.")
    );
  } catch (err) {
    next(err);
  }
...
export async function muteParticipant(
  ...
  try {
    ...
    res.status(HTTP_STATUS.OK).json(
      successResponse(null, "Participant muted.")
    );
  } catch (err) {
    next(err);
  }
...
export async function kickParticipant(
  ...
  try {
    ...
    res.status(HTTP_STATUS.OK).json(
      successResponse(null, "Participant removed from voice channel.")
    );
  } catch (err) {
    next(err);
  }
...
export async function endVoiceSession(
  ...
  try {
    ...
    res.status(HTTP_STATUS.OK).json(
      successResponse(null, "Voice session ended.")
    );
  } catch (err) {
    next(err);
  }
...
export async function reportQualityStats(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
